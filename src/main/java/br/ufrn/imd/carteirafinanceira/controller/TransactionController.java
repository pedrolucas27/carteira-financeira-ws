...
public class TransactionController {
    ...
    @PostMapping(value = "speding", consumes = { MediaType.APPLICATION_JSON_VALUE }, produces = {
    ...
    public ResponseEntity<String> registerSpeding(@RequestBody Spending spending) {
        ...
        if (result)
            return new ResponseEntity<>("despesa registrada com sucesso!", HttpStatus.OK);
        return new ResponseEntity<String>("erro ao efetuar transação!", HttpStatus.BAD_REQUEST);
    }

    // register Proof
    @PostMapping(value = "proof", consumes = { MediaType.APPLICATION_JSON_VALUE }, produces = {
            MediaType.APPLICATION_JSON_VALUE })
    public ResponseEntity<String> registerProof(@RequestBody Proof proof) {
        ...
        if (result)
            return new ResponseEntity<>("receita registrada com sucesso!", HttpStatus.OK);
        return new ResponseEntity<String>("erro ao efetuar transação!", HttpStatus.BAD_REQUEST);
    }

    @GetMapping("{cpf}")
    public ResponseEntity<List<TransactionDAO.TransactionResponse>> listAllByCpfUser(@PathVariable String cpf) {
        List<TransactionDAO.TransactionResponse> result = service.listAllByCpfUser(cpf);
